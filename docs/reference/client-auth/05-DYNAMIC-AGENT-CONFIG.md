...
1. User sends request with Bearer token
   │
   ▼
2. MastraAuthEntra.authenticateToken(token)
   - Verifies JWT signature via JWKS
   - Extracts user claims including groups
   │
   ▼
3. Middleware injects user into RequestContext
   c.set('requestContext', { user, userToken, userId })
   │
   ▼
4. Agent configuration functions execute:
   │
   ...
   │
   ▼
5. LLM receives context with ONLY accessible resources
   - Cannot reference finance tools if user is in Engineering
   - Cannot reveal existence of restricted resources
   │
   ▼
6. LLM generates response using available capabilities
```
