...
In the BFF architecture, this provider verifies tokens that the BFF proxy attaches to internal requests. The tokens themselves are stored server-side and never exposed to the browser.
...
export class MastraAuthEntra extends MastraAuthProvider<EntraUser> {
  ...
  async authenticateToken(token: string): Promise<EntraUser | null> {
    try {
      ...
      if (payload.iss !== this.issuer) {
        console.warn('Token issuer mismatch:', payload.iss, 'expected:', this.issuer);
        return null;
      }

      // Validate audience matches our application
      if (payload.aud !== this.clientId) {
        console.warn('Token audience mismatch:', payload.aud, 'expected:', this.clientId);
        return null;
      }
  ...
  static getUserDisplayName(user: EntraUser): string {
    return user.name || user.preferred_username || user.email || user.oid || 'Unknown User';
  }
}
```
  ...
  try {
    ...
    while (response) {
      const groupIds = response.value
        .filter((member: any) => member['@odata.type'] === '#microsoft.graph.group')
...
export { fetchUserGroupsFromGraph, resolveUserGroups } from './entra-groups-resolver';
```
...
// Get a token from your BFF session (log it during development)
...
6. **Tokens stay server-side** - In BFF pattern, browser never sees tokens
